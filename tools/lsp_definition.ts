...
// .opencode/tools/lsp_definition.ts
...
function isLocationLink(obj) {
  return "targetUri" in obj;
}
function extractLocationInfo(loc) {
...
function formatLocation(loc) {
  const { uri, range } = extractLocationInfo(loc);
  const filePath = uriToFilePath(uri);
  const line = range.start.line + 1;
  const character = range.start.character + 1;
  return `${filePath}:${line}:${character}`;
}
var lsp_definition_default = tool({
  description: "Find where a symbol is defined using the language server. Returns the file path, line, and character of the definition. Works for Go (gopls), TypeScript/JavaScript (typescript-language-server), Python (ty), and Rust (rust-analyzer).",
  args: {
...
export {
  lsp_definition_default as default
};
