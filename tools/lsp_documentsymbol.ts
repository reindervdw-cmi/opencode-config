...
// .opencode/tools/lsp_documentsymbol.ts
...
function formatSymbolInformation(symbol) {
  ...
  return `${kindName} ${symbol.name}${container} \u2014 ${filePath}:${line}:${character}`;
}
var lsp_documentsymbol_default = tool({
  description: "Get all symbols (functions, classes, variables, etc.) in a file using the language server. Returns a structured list of symbols with their kind, name, and location. Works for Go (gopls), TypeScript/JavaScript (typescript-language-server), Python (ty), and Rust (rust-analyzer).",
  args: {
  ...
  async execute(args, context) {
    try {
      ...
      if (isDocumentSymbol(firstItem)) {
        const symbols = result;
        const formatted = symbols.map((symbol) => formatDocumentSymbol(symbol, 0)).join("\n");
        return `Found ${symbols.length} symbol(s):
${formatted}`;
      } else {
        const symbols = result;
        const formatted = symbols.map(formatSymbolInformation).join("\n");
        return `Found ${symbols.length} symbol(s):
${formatted}`;
      }
    } catch (error) {
...
export {
  lsp_documentsymbol_default as default
};
