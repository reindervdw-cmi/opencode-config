...
// .opencode/tools/lsp_workspacesymbol.ts
...
function getFirstWorkspaceFile(worktree) {
  const extensions = [".ts", ".go", ".py", ".rs", ".js"];
  try {
    const entries = readdirSync(worktree);
    for (const ext of extensions) {
      const match = entries.find((e) => e.endsWith(ext));
      if (match) return path.join(worktree, match);
    }
  } catch {
  }
  return null;
...
function formatSymbol(symbol) {
  ...
  const containerPart = symbol.containerName ? ` in ${symbol.containerName}` : "";
  return `${kindName} ${symbol.name}${containerPart} \u2014 ${filePath}:${line}:${character}`;
}
var lsp_workspacesymbol_default = tool({
  description: "Search for symbols (functions, classes, variables, etc.) across the entire workspace using the language server. Returns matching symbols with their kind, name, file path, and location. Works for Go (gopls), TypeScript/JavaScript (typescript-language-server), Python (ty), and Rust (rust-analyzer).",
  args: {
    query: tool.schema.string().describe(
      'The search query (e.g., a class or function name). An empty string "" returns all symbols.'
    ),
    file: tool.schema.string().optional().describe(
      "Absolute path to any file in the workspace, used to determine which language server to start. When omitted, auto-detects from workspace root."
    )
  },
  async execute(args, context) {
    try {
      let filePath;
      if (args.file) {
        filePath = path.resolve(context.worktree, args.file);
      } else {
        const detectedFile = getFirstWorkspaceFile(context.worktree);
        if (!detectedFile) {
          return `Could not determine language server: no supported source files found in workspace root. Provide a 'file' argument.`;
        }
      ...
      const formattedSymbols = result.map(formatSymbol).join("\n");
      return `Found ${result.length} symbol(s) matching "${args.query}":
${formattedSymbols}`;
    } catch (error) {
...
export {
  lsp_workspacesymbol_default as default
};
