...
function normalizeWorkspaceEdit(edit) {
  ...
  if (edit.changes) {
    for (const [uri, edits] of Object.entries(edit.changes)) {
      const existing = fileEdits.get(uri) ?? [];
      fileEdits.set(uri, [...existing, ...edits]);
    }
  }
  if (edit.documentChanges) {
    for (const change of edit.documentChanges) {
      ...
      const existing = fileEdits.get(uri) ?? [];
      fileEdits.set(uri, [...existing, ...change.edits]);
    }
  }
  return fileEdits;
}
function applyEdits(fileEdits) {
  const modifiedFiles = [];
...
var lsp_rename_default = tool({
  description: "Rename a symbol across the entire workspace using the language server. Returns a summary of all files and lines that would be changed. Use the apply argument to actually apply the changes to disk. Works for Go (gopls), TypeScript/JavaScript (typescript-language-server), Python (ty), and Rust (rust-analyzer).",
  args: {
    file: tool.schema.string().describe("Absolute path to the file containing the symbol to rename"),
    line: tool.schema.number().int().min(0).describe("Zero-based line number of the symbol"),
    character: tool.schema.number().int().min(0).describe("Zero-based character offset of the symbol on the line"),
    newName: tool.schema.string().min(1).describe("The new name for the symbol"),
    apply: tool.schema.boolean().optional().describe(
      "If true, apply the rename edits to disk. If false (default), only preview the changes."
    )
  },
  async execute(args, context) {
    try {
      ...
      if (!result) {
        return "No rename edits returned by language server.";
      }
      const fileEdits = normalizeWorkspaceEdit(result);
      if (fileEdits.size === 0) {
        return "No rename edits returned by language server.";
      }
      if (args.apply === true) {
        const modifiedFiles = applyEdits(fileEdits);
        for (const modifiedFile of modifiedFiles) {
          await client.syncDocument(modifiedFile);
        }
        const fileList = modifiedFiles.map((f) => `  - ${f}`).join("\n");
        return `Renamed to '${args.newName}'. Applied changes to ${modifiedFiles.length} file(s):
${fileList}`;
      }
      const fileLines = [];
      for (const [uri, edits] of fileEdits) {
        const fp = uriToFilePath(uri);
        fileLines.push(`  ${fp}: ${edits.length} edit(s)`);
      }
      return `Rename preview: '${args.newName}'

Changes in ${fileEdits.size} file(s):
${fileLines.join("\n")}
