...
// .opencode/tools/lsp_references.ts
...
var lsp_references_default = tool({
  description: "Find all references to a symbol across the workspace using the language server. Returns file paths, lines, and character positions of every usage. Works for Go (gopls), TypeScript/JavaScript (typescript-language-server), Python (ty), and Rust (rust-analyzer).",
  args: {
    ...
    includeDeclaration: tool.schema.boolean().optional().describe(
      "Whether to include the symbol's declaration in the results (default: true)"
    )
  },
  async execute(args, context) {
    try {
      ...
      const formattedLocations = result.map(formatLocation).join("\n");
...
export {
  lsp_references_default as default
};
