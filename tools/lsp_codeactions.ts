...
// .opencode/tools/lsp_codeactions.ts
import path from "node:path";
import { readFileSync as readFileSync2, writeFileSync } from "node:fs";
import { tool } from "@opencode-ai/plugin";

// .opencode/tools/lib/language-config.ts
...
function getLanguageServerConfig(filePath) {
...
var LspClient = class {
  ...
  /**
   * Start the language server process and initialize the LSP connection.
   */
  async start() {
    ...
    this.process.on("exit", (code, signal) => {
      if (this.isReady) {
        console.error(
          `[lsp-client] Language server ${this.config.id} exited unexpectedly: code=${code}, signal=${signal}`
        );
      }
    });
    this.process.on("error", (err) => {
      console.error(
        `[lsp-client] Language server ${this.config.id} error: ${err.message}`
      );
    });
    this.process.stderr?.on("data", (data) => {
      console.error(`[${this.config.id}] ${data.toString()}`);
    });
    ...
    this.connection = (0, import_node.createMessageConnection)(
      new import_node.StreamMessageReader(this.process.stdout),
      new import_node.StreamMessageWriter(this.process.stdin)
    );
    this.connection.listen();
    const rootUri = filePathToUri(this.workspaceRoot);
    const capabilities = {
      textDocument: {
   ...
   * Reads the file from disk and sends textDocument/didOpen notification.
   */
  ...
  /**
   * Sync a document that was modified externally (e.g., after applying edits).
   * Closes and reopens the document to refresh the language server's view.
   */
  async syncDocument(filePath) {
    ...
    this.connection.sendNotification("textDocument/didChange", changeParams);
    this.openedDocuments.set(uri, newVersion);
  }
  /**
  ...
  async closeDocument(filePath) {
    ...
    this.connection.sendNotification("textDocument/didClose", closeParams);
...
// .opencode/tools/lib/server-pool.ts
var DEFAULT_IDLE_TIMEOUT_MS = 5 * 60 * 1e3;
function makeKey(config, workspaceRoot) {
  return `${config.id}::${workspaceRoot}`;
}
var ServerPool = class {
  ...
  /**
   * Get a client for the given config and workspace root.
   *
   * If an existing client is available, resets its idle timer and returns it.
   * Otherwise, spawns a new client, starts it, and adds it to the pool.
   */
  async getClient(config, workspaceRoot) {
    const key = makeKey(config, workspaceRoot);
    const existing = this.pool.get(key);
    if (existing) {
      clearTimeout(existing.idleTimer);
      existing.lastUsed = Date.now();
      existing.idleTimer = this.createIdleTimer(key, config, workspaceRoot);
    ...
    const entry = {
      client,
      lastUsed: Date.now(),
      idleTimer: this.createIdleTimer(key, config, workspaceRoot)
    };
    this.pool.set(key, entry);
    return client;
  }
  /**
   * Explicitly release/shutdown a client and remove from pool.
   * Does nothing if the client is not found.
   */
  async releaseClient(config, workspaceRoot) {
    const key = makeKey(config, workspaceRoot);
  ...
  /**
   * Shutdown all clients in the pool and clear it.
   */
  async shutdown() {
    const shutdownPromises = [];
    for (const [key, entry] of this.pool) {
      clearTimeout(entry.idleTimer);
      shutdownPromises.push(entry.client.shutdown());
    }
    this.pool.clear();
    await Promise.all(shutdownPromises);
  }
  /**
   * Return the number of active entries in the pool.
   */
  get size() {
    return this.pool.size;
  }
  /**
   * Create an idle timer that shuts down the client when it expires.
   */
  createIdleTimer(key, config, workspaceRoot) {
    return setTimeout(async () => {
      const entry = this.pool.get(key);
      if (entry) {
        this.pool.delete(key);
        console.error(
          `[server-pool] Idle timeout: shutting down ${config.id} for ${workspaceRoot}`
        );
        await entry.client.shutdown();
      }
    }, this.idleTimeoutMs);
...
function applyTextEdit(content, edit) {
  const lines = content.split("\n");
  const { start, end } = edit.range;
  const beforeLines = lines.slice(0, start.line);
  const beforePartial = lines[start.line]?.slice(0, start.character) ?? "";
  const afterPartial = lines[end.line]?.slice(end.character) ?? "";
  const afterLines = lines.slice(end.line + 1);
  const newLines = edit.newText.split("\n");
  const resultLines = [...beforeLines];
  if (newLines.length === 1) {
    resultLines.push(beforePartial + newLines[0] + afterPartial);
  } else {
    resultLines.push(beforePartial + newLines[0]);
    resultLines.push(...newLines.slice(1, -1));
    resultLines.push(newLines[newLines.length - 1] + afterPartial);
  }
  resultLines.push(...afterLines);
  return resultLines.join("\n");
}
function sortEditsReverse(edits) {
...
function applyWorkspaceEdit(edit) {
  ...
  if (edit.changes) {
    for (const [uri, edits] of Object.entries(edit.changes)) {
      ...
      fileEdits.set(filePath, [...existing, ...edits]);
    }
  }
  if (edit.documentChanges) {
    for (const change of edit.documentChanges) {
      ...
      const existing = fileEdits.get(filePath) ?? [];
      fileEdits.set(filePath, [...existing, ...change.edits]);
    }
  }
...
var lsp_codeactions_default = tool({
  description: "Get available code actions (quick fixes, refactorings) for a range in a file using the language server. Returns a numbered list of available actions. Optionally execute a specific action by its number. Works for Go (gopls), TypeScript/JavaScript (typescript-language-server), Python (ty), and Rust (rust-analyzer).",
  args: {
    file: tool.schema.string().describe("Absolute path to the file"),
    startLine: tool.schema.number().int().min(0).describe("Zero-based start line of the range"),
    startCharacter: tool.schema.number().int().min(0).describe("Zero-based start character of the range"),
    endLine: tool.schema.number().int().min(0).describe("Zero-based end line of the range"),
    endCharacter: tool.schema.number().int().min(0).describe("Zero-based end character of the range"),
    execute: tool.schema.number().int().min(1).optional().describe(
      "If provided, execute the code action with this number from the list (1-indexed)"
    )
  },
  async execute(args, context) {
    try {
      const filePath = path.resolve(context.worktree, args.file);
      const config = getLanguageServerConfig(filePath);
      if (!config) {
        const ext = path.extname(filePath) || "(no extension)";
        return `Unsupported file type: ${ext}. Supported: .go, .ts, .tsx, .js, .jsx, .py, .pyi, .rs`;
      }
      const client = await serverPool.getClient(config, context.worktree);
      ...
      if (args.execute === void 0) {
        const lines = result.map((item, index) => {
          const num = index + 1;
          const title2 = item.title;
          const kind = isCodeAction(item) ? item.kind : void 0;
          if (kind) {
            return `${num}. [${kind}] ${title2}`;
          }
          return `${num}. ${title2}`;
        });
...
${lines.join("\n")}`;
      ...
      const actionIndex = args.execute - 1;
      if (actionIndex < 0 || actionIndex >= result.length) {
        return `Invalid action number. Available: 1-${result.length}`;
      }
      const selectedItem = result[actionIndex];
      const title = selectedItem.title;
      if (isCodeAction(selectedItem)) {
        const action = selectedItem;
        const hasEdit = action.edit !== void 0;
        const hasCommand = action.command !== void 0;
        if (!hasEdit && !hasCommand) {
          return `Action '${title}' has no edit or command to apply.`;
        }
        const modifiedFiles = [];
        if (hasEdit && action.edit) {
          const files = applyWorkspaceEdit(action.edit);
          modifiedFiles.push(...files);
          for (const modifiedFile of files) {
            await client.syncDocument(modifiedFile);
          }
        }
        ...
        if (hasEdit && modifiedFiles.length > 0) {
          const fileList = modifiedFiles.map((f) => `  - ${f}`).join("\n");
          return `Executed: '${title}'. Applied changes to ${modifiedFiles.length} file(s):
${fileList}`;
        } else if (hasCommand) {
          return `Executed command: '${title}'`;
        }
      ...
      return `LSP error: ${message}`;
    }
  }
});
export {
  lsp_codeactions_default as default
};
