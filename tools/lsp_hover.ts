...
// .opencode/tools/lsp_hover.ts
function isMarkupContent(contents) {
  return typeof contents === "object" && !Array.isArray(contents) && "kind" in contents && "value" in contents;
}
function isMarkedStringWithLanguage(contents) {
  return typeof contents === "object" && "language" in contents && "value" in contents;
}
function extractMarkedString(marked) {
  if (typeof marked === "string") {
    return marked;
  }
  if (isMarkedStringWithLanguage(marked)) {
...
function extractHoverContent(contents) {
  if (isMarkupContent(contents)) {
    return contents.value;
  }
  if (Array.isArray(contents)) {
    return contents.map(extractMarkedString).join("\n\n---\n\n");
  }
  return extractMarkedString(contents);
}
var lsp_hover_default = tool({
  description: "Get type information and documentation for a symbol at a given position using the language server. Returns type signatures, documentation, and other hover details as provided by the language server. Works for Go (gopls), TypeScript/JavaScript (typescript-language-server), Python (ty), and Rust (rust-analyzer).",
  args: {
    file: tool.schema.string().describe("Absolute path to the file containing the symbol"),
